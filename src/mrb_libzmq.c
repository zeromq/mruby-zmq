...
mrb_zmq_thread_fn(void *mrb_zmq_thread_data_)
{
  ...
  if (likely(mrb)) {
    ...
    MRB_TRY(&c_jmp)
    {
      ...
      argv = mrb_funcall(mrb, mrb_obj_value(mrb_module_get(mrb, "MessagePack")), "unpack", 1, argv_str);
...
mrb_zmq_threadstart(mrb_state *mrb, mrb_value thread_class)
{
  ...
  MRB_CATCH(&c_jmp)
  {
    ...
    if (backend && !success) {
      int disable = 0;
      zmq_setsockopt(backend, ZMQ_SNDTIMEO, &disable, sizeof(disable));
      zmq_setsockopt(backend, ZMQ_LINGER, &disable, sizeof(disable));
      zmq_close(backend);
    }
    MRB_THROW(mrb->jmp);
  }
